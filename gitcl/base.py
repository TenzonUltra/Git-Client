...
def init ():
    data.init ()
    # init() should happen once
    if (data.GIT_DIR /'refs/heads/master').is_file():
        return
    data.update_ref ('HEAD', data.RefValue (symbolic=True, value='refs/heads/master'))


def write_tree ():
    # Get Index as a tree of dicts
    index_as_tree = {}
...
def is_ignored (path):
    ...
    # dirs to exclude
    for s in [ '.gitignore', '.gitcl', 'gitcl.egg-info']:
        if s in path.parts:
            return True
